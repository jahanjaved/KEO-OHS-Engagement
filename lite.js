(() => {
  ...
  function runEngine(card){
    ...
    if(!manual?.value.trim() || !btn) {
      compose(card);
      updateAutoStatus(card,'Smart engagement analysis ready');
      return;
    }
    updateAutoStatus(card,'Preparing engagement description and recommendations…',true);
    btn.click();
    setTimeout(()=>{
      compose(card);
      updateAutoStatus(card,'Engagement description and recommendations generated');
    },40);
  }
